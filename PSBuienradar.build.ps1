...
<#
.SYNOPSIS
    Build script for the 'PSBuienradar' PowerShell module

.DESCRIPTION
    This script contains the tasks for building the 'PSBuienradar' PowerShell module
#>
[CmdletBinding()]
...
Enter-Build {
    $script:moduleName = 'PSBuienradar'
...
Task Export-CommandHelp {
    ...
    $helpFiles = Measure-PlatyPSMarkdown -Path "$buildPath/help/PSBuienradar/*.md"
    foreach ($helpFile in $helpFiles) {
        [void] (Test-MarkdownCommandHelp -Path $helpFile.FilePath)
    }

    # Generate module help file
    $mdfiles = Measure-PlatyPSMarkdown -Path "$buildPath/help/PSBuienradar/*.md" | Where-Object Filetype -Match 'CommandHelp'
